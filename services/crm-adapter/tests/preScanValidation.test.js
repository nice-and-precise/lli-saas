...
describe("pre-scan Monday validation", () => {
  it("returns validation details from /boards/validate for board, mapping, and token readiness", async () => {
    ...
    const response = await request(app).get("/boards/validate");

    expect(response.statusCode).toBe(200);
    expect(response.body.valid).toBe(true);
    expect(response.body.ready).toBe(true);
    expect(response.body.token_validation.refresh.status).toBe("not_supported");
